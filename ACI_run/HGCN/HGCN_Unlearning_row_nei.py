...
1) 加载训练数据、构造超图并训练 HyperGCN；
2) 在训练完成后，从训练数据中随机选取 a% 节点作为“待删除”节点，使用 GIF 方法进行 Unlearning；
3) 在独立的测试数据集上评估删除前后的模型性能差异（micro‑F1）。
"""
...
def retrain_after_prune(
    ...
    print(f"set data: {X_keep.shape[0]}")
    ...
    model_re.structure = model_re.structure.to(device)
    for layer in model_re.layers:
        layer.reapproximate = False


    rem = {"x": fts_keep, "y": lbls_keep, "train_mask": mask_keep}
    f1_rem, acc_rem = evaluate_hgcn_f1(model_re, rem), evaluate_hgcn_acc(model_re, rem)
    ...
    test = {"x": fts_te, "y": lbls_te}
    f1_test, acc_test = evaluate_hgcn_f1(model_re, test), evaluate_hgcn_acc(model_re, test)
    ...
    print("— Membership Inference Attack on Retrained Model —")

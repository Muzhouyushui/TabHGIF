...
def retrain_on_pruned(X, y, df, transformer, deleted_idx, args, device):
    ...
    Additional outputs now include:
      - Number of training nodes after pruning
    ...
    print(
        f"Number of hyperedges in deleted set: {num_hyperedges}, deleted-set node features not zeroed out? {has_nonzero_features}")
...
def full_train_and_unlearn(X, y, df, transformer, deleted_idx, args, device):
    ...
    print(f"GIF Unlearning 以前: F1={f1_before:.4f}, Acc={acc_before:.4f}")
...
def batch_main(runs=20):
    ...
    del_count = int(del_ratio * num_train)
    ...
    for run in range(1, runs+1):
        print(f"\n=== Run {run}/{runs} ===")
        deleted_idx = np.random.choice(np.arange(num_train), size=del_count, replace=False)

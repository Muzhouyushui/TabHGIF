...
---------------------
Single-graph version of the Hypergraph Attention Layer:
...
def segment_softmax(src: torch.Tensor, index: torch.Tensor, num_seg: int):
    """
    Group-wise softmax; no need to compute max first, directly normalize e^x.
    """
...
class HyperGraphAttentionLayerSparse(nn.Module):
    """
    Interface remains exactly the same as the original: forward(x, H) -> out
    Only eps is added during degree normalization to avoid inf/nan caused by division by zero.
    """
    def __init__(self,
                 in_features,
                 out_features,
    ...
    def forward(self, x: torch.Tensor, H: torch.sparse.FloatTensor) -> torch.Tensor:
        ...
        N = x.size(0)
        # —— Automatically correct the direction of H —— #
        H_inc = H if H.size(0) == N else H.transpose(0, 1)

        # —— 1) Compute degree vectors —— #
        #    The original version may be dv_inv = dv.pow(-0.5), de_inv = de.pow(-1.0)
        #    Here eps is added to avoid zero degrees
        eps = 1e-10

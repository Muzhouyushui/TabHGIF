...
def main():
    ...
    remove_ratio = getattr(args, 'remove_ratio', 0.30)  # 默认删 30%
    ...
    # ---- Deleted nodes ACC before unlearning (on training graph) ----
    model.eval()
    with torch.no_grad():
        logits_train = model(X_train_t, H_train)        # [N_train, C]
        preds_train  = logits_train.argmax(dim=1)       # [N_train]
        del_acc_before = accuracy_score(
            y_train_t[deleted_idx].cpu(),
            preds_train[deleted_idx].cpu()
        )
    print(f"Deleted nodes ACC before unlearning: {del_acc_before:.4f}")
    ...
    # ---- Deleted nodes ACC after unlearning (on training graph) ----
    model.eval()
    with torch.no_grad():
        logits_train_un = model(X_train_t, H_train)     # [N_train, C]
        preds_train_un  = logits_train_un.argmax(dim=1) # [N_train]
        del_acc_after = accuracy_score(
            y_train_t[deleted_idx].cpu(),
            preds_train_un[deleted_idx].cpu()
        )
    print(f"Deleted nodes ACC after unlearning:  {del_acc_after:.4f}")

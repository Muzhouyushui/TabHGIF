...
from GIF.GIF_HGNN_ROW_NEI import (
...
def full_train_and_unlearn(X, y, df, transformer, deleted_idx, args, device):
    ...
    print(f"[Deleted-Node Evaluation] HGIF scores on deleted nodes: F1={f1_del:.4f}, Acc={acc_del:.4f}")
...
def batch_main(runs=20):
    ...
    for i in range(1, runs+1):
        print(f"\n=== Run {i}/{runs} ===")
        deleted_idx = np.random.choice(num_train, size=del_count, replace=False)
    ...
    df.to_csv("batch_unlearning_results.csv", index=False)
    return df, summary

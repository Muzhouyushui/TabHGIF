...
"""
HGNNP FT baselines on edited hypergraph (COLUMN unlearning, feature-column deletion), in the same style as your HGCN/HGNN FT scripts.
...
# ===== Column-level preprocessing / deletion (same family as your HGNNP_Unlearning_col.py) =====
...
# -----------------------------
# Loss (keep same style as your HGNNP row FT baseline: FocalLoss)
# -----------------------------
...
def run_one(args, run_id: int):
    ...
    # ===== 1) Load train/test (column pipeline) =====
    ...
    print(f"标签分布(Train) → <=50K: {train_counter.get(0,0)}，>50K: {train_counter.get(1,0)}")
    print(f"标签分布(Test)  → <=50K: {test_counter.get(0,0)}，>50K: {test_counter.get(1,0)}")

    # ===== 2) Build original hyperedges / structures =====
    ...
    fts_tr = torch.FloatTensor(X_train).to(device)
    lbls_tr = torch.LongTensor(y_train).to(device)

    fts_te = torch.FloatTensor(X_test).to(device)
    ...
    # train side delete
    fts_tr_edit, H_tr_edit_from_delete, hyperedges_tr_edit = delete_feature_column(
    ...
    # test side delete
    fts_te_edit, H_te_edit_from_delete, hyperedges_te_edit = delete_feature_column(
        fts_te.clone(), transformer, col_to_remove,
        H_te, hyperedges_te,
        continuous_cols=args.continuous_cols
    )

    # rebuild HGNNP structures from edited hyperedges (recommended for consistency)
    ...
    rows = []
    mia_full = maybe_mia("Full@EditedHG", model_full, np.asarray(X_train), np.asarray(y_train), hyperedges_tr_edit, args, device)
    rows.append({
        "run": run_id,
        "seed": seed,
    ...
    for K in args.ft_steps:
        ...
        mia_o = maybe_mia(f"FT-K(K={K})", m, np.asarray(X_train), np.asarray(y_train), hyperedges_tr_edit, args, device)
        rows.append({
            "run": run_id,
            "seed": seed,
    ...
    for K in args.ft_steps:
        ...
        mia_o = maybe_mia(f"FT-head(K={K})", m, np.asarray(X_train), np.asarray(y_train), hyperedges_tr_edit, args, device)
        rows.append({
            "run": run_id,
            "seed": seed,
...
# -----------------------------
def build_parser():
    p = argparse.ArgumentParser("HGNNP FT baseline on edited hypergraph (column unlearning)")
    ...
    # ===== Column preprocessing settings =====
    p.add_argument("--cat_cols", type=str, nargs="+", default=[
        "workclass", "education", "marital-status", "occupation",
        "relationship", "race", "sex", "native-country"
    ])
    # 连续列按你的 data_preprocessing_column 实现对应 Adult 配置来
    ...
    p.add_argument("--run_mia", action="store_true", default=False)  # 列级先默认关
    p.add_argument("--out_csv", type=str, default="ft_hgnnp_col_zero_results.csv")

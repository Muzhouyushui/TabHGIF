...
from bank.HGNN.data_preprocessing_Balancetry import (
...
def retrain_on_pruned(X, y, df, transformer, deleted_idx, args, device):
    ...
    Now also outputs:
      ...
      - Test-set F1 & Acc
      1) Delete nodes specified by deleted_idx and retrain HGNN;
    2) Evaluate once on the full test set (F1/Acc);
    3) On the deleted nodes, make predictions with orig_model and retrained_model respectively,
    ...
    print(f"[Deleted-node Evaluation] Scores of the retrained model on deleted nodes:: F1={f1_del:.4f}, Acc={acc_del:.4f}")

    # ——— New: perform MIA evaluation on the retrained model ———
    # Make sure membership_inference and generate_hyperedge_dict have been imported
    ...
    # 3) Merge hyperedges (del part needs offset)
    he_attack = {}
    cnt = 0
    ...
    member_mask = np.concatenate([
        np.ones(len(X_keep), dtype=bool),  # remaining nodes are treated as positive examples
        np.zeros(len(X_del), dtype=bool)  # deleted nodes are treated as negative examples
    ])
...
def full_train_and_unlearn(X, y, df, transformer, deleted_idx, args, device):
    ...
    def reason_once_unlearn(data):
        # If it is necessary to zero out data["x"] or rebuild the structure, modify it here
        return model(
            data["x"],
            data["H"],
            data["dv_inv"],
    ...
    # —— New: compute the number of misclassifications on the test set —— #
    with torch.no_grad():
        ...
        num_errors = mismatches.sum().item()  # number of classification errors
    ...
    unlearning_time, _ = approx_gif(
        model,
        data_obj,
        (deleted_idx, hyperedges, 13),   # <<< pass in hyperedges instead of None
    ...
    # 3) Merge hyperedges (node indices in the del part need offsetting)
    he_attack = {}
    idx_he = 0
    for nodes in he_keep.values():
    ...
    y_attack = np.hstack([y_keep, y_del])
    member_mask = np.concatenate([
        np.ones(len(X_keep), dtype=bool),   # keep nodes are treated as positive examples
        np.zeros(len(X_del),  dtype=bool)   # del nodes are treated as negative examples
    ])
...
def main():
    ...
    # 3) Randomly select node indices to delete
    ...
    retrained_model = retrain_on_pruned(
        X            = X_tr,
        y            = y_tr,
        df           = df_tr,         # pass in the original df_tr to preserve categorical columns
    ...
    _ = full_train_and_unlearn(
        X            = X_tr,
        y            = y_tr,
        df           = df_tr,         # likewise pass in the original df_tr
        transformer  = transformer,
        deleted_idx  = deleted_idx,
        args         = args,
        device       = device

...
from Credit.HGCN.data_preprocessing_balanced import (
...
def retrain_after_prune(
    ...
    # 4) Prepare tensors and initialize model
    ...
    # Disable reapproximation in each layer
    ...
    # Pass feature fts_te directly to match the evaluate_hgcn_* interface
...
def main():
    parser = argparse.ArgumentParser(description="HyperGCN on Credit Approval dataset with node-level unlearning")
    ...
    parser.add_argument("--neighbor-k", type=int, default=12, help="Hyperedge share threshold K")
    ...
    # 4) Preprocess train/test with existing transformer
    ...
    # 4) Initialize and train the full model
    ...
    # 4.3 Initialize HyperGCN and fix Laplacian
    model = HyperGCN(
        X_tr.shape[0],
        hyperedges_tr,
        X_tr,
    ...
    # 5) Randomly delete nodes
    deleted = torch.tensor(
        np.random.choice(len(y_tr), int(len(y_tr) * args.remove_ratio), replace=False),
    ...
    # 7.2) Restore original training structure & evaluate before deletion
    model.structure = A_before
    for layer in model.layers:
        layer.reapproximate = False
    ...
    print("Check whether all deleted-node features are zero:", (fts_new[deleted] != 0).sum().item())

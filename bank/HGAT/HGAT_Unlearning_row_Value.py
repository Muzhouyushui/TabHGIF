...
def main():
    ...
    mask = (
            (df_train_proc['marital'] == 'married') &
            (df_train_proc['housing'] == 'yes')
    )
    deleted_idx_np = np.where(mask.values)[0]  # numpy indices
    deleted_idx = torch.tensor(deleted_idx_np, dtype=torch.long, device=device)
    num_train = X_train_t.size(0)
    num_del = deleted_idx_np.shape[0]
    pct = 100.0 * num_del / num_train
    print(f"Delete by value: {num_del}/{num_train} nodes in total (≈{pct:.1f}%), first 5 indices: {deleted_idx_np[:5]}")
    ...
    # Construct member_mask: retained nodes are 1 (positive), deleted nodes are 0 (negative)
    num_samples = X_train_np.shape[0]
    member_mask = np.ones(num_samples, dtype=bool)
    member_mask[deleted_idx_np] = False
    ...
    attack_model, (auc_s, f1_s), (auc_t, f1_t) = membership_inference_hgat(
        ...
        y_train_np,
        train_edges,
        target_model=model,   # pass in the already unlearned model here
        args=args,
        device=device,

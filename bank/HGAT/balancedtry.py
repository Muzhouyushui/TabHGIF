...
# Data tools
from bank.HGAT.data_preprocessing_Balancetry import (
...
def main():
    ...
    # 4) Call preprocessing with DataFrame (no temporary CSV needed)
    ...
    # This part should be executed after the model training above is completed.
    # Assume you have already loaded X_train_t, y_train_t, H_train, train_edges, device, and args

    # 1) Compute the number of nodes to delete
    num_train = X_train_t.size(0)
    remove_ratio = getattr(args, 'remove_ratio', 0.30)  # Default: delete 30%
    num_del = int(remove_ratio * num_train)

    # 2) Randomly sample nodes to be deleted
    # Option 1: use torch.randperm
    perm = torch.randperm(num_train, device=device)
    deleted_idx = perm[:num_del]  # LongTensor [num_del]
    deleted_idx_np = deleted_idx.cpu().numpy()  # numpy array

    # Or option 2: use numpy.choice
    # deleted_idx_np = np.random.choice(num_train, num_del, replace=False)
    # deleted_idx    = torch.from_numpy(deleted_idx_np).to(device)
    ...
    unlearn_info = (
        deleted_idx_np,  # numpy array for find_hyperneighbors
        train_edges,
        K  #
    )
    ...
    model.eval()
    with torch.no_grad():
        # Use the original test hypergraph here, instead of H_un after deletion on the training set

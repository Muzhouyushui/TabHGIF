...
def retrain_after_prune(
    ...
    """
    # 1) Compute indices of nodes to keep — ensure deleted is on the same device
    ...
    # Disable recomputation in every layer
...
def main():
    ...
    print("Check whether deleted-node features are all zero:", (fts_new[deleted] != 0).sum().item())
    print(f" Deleted {len(deleted)} nodes; {len(edge_list_new)} hyperedges remain")

    # 7.4) GIF update (simultaneously compensating deleted nodes and their neighbors)
    gif_time = approx_gif(
        model, data_train,
        A_before, A_after,
        deleted_nodes=deleted,

...
# hgnn_module.py  —— 其余部分保持不变

class HGNN_conv_implicit(nn.Module):
    """
    Single-layer implicit hypergraph convolution.
    y = (Dv^{-1/2} H De^{-1} H^T Dv^{-1/2} + I) x W + b
    （在原来聚合完再做线性之前，加了一个 x 的自环分量）
    """
    ...
    def forward(self,
                ...
                dv_inv: torch.Tensor,
                de_inv: torch.Tensor) -> torch.Tensor:
        # 原 HGNN 的聚合步骤：y = Dv^{-1/2} H De^{-1} H^T Dv^{-1/2} x
        x_norm = x * dv_inv.unsqueeze(1)
        ...
        # 再做线性映射 + bias
        out = y_hat.matmul(self.weight)
        if self.bias is not None:
            out = out + self.bias
        return out


class HGNN_implicit(nn.Module):
    """
    两层隐式 HGNN，接口与原版一致
    """
    def __init__(self, in_ch: int, n_class: int, n_hid: int,
                 dropout: float = 0.5):
        super().__init__()
        self.hgc1    = HGNN_conv_implicit(in_ch,  n_hid)
        self.hgc2    = HGNN_conv_implicit(n_hid,  n_class)
        self.dropout = dropout

    def forward(self, x: torch.Tensor, H: torch.sparse.FloatTensor,
                dv_inv: torch.Tensor, de_inv: torch.Tensor) -> torch.Tensor:
        x = F.relu(self.hgc1(x, H, dv_inv, de_inv))

...
def preprocess_node_features_bank(
    data_source,            # Can be either a "file path" (str) or an existing DataFrame
    is_test: bool = False,
    transformer: ColumnTransformer = None
):
    """
    Load and preprocess the Bank dataset. If data_source is a str, it will be read as a CSV path;
...
def generate_hyperedge_dict_bank(
    ...
    Return:
      hyperedges (dict) with keys as (col, val[, cluster_id]) and values as lists of node indices
    """
    hyperedges = {}
    col_edge_count = {}

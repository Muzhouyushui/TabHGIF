...
class AttackModel(nn.Module):
    """
    A simple two-layer binary attack model.
    """
...
def train_shadow_model(model: nn.Module,
    ...
    """
    Train a Shadow HGCN with NLLLoss and ensure its structure
    (Laplacian) is on the same device.
    """
    device = data.x.device
    model.to(device)

    # Ensure model.structure is a sparse tensor and placed on GPU
    if isinstance(model.structure, list):
        ...
        # If it is already a tensor, directly move it to device
        model.structure = model.structure.to(device)

    # Optionally update cached structure settings in HyperGCN layers
    for layer in getattr(model, "layers", []):
        if hasattr(layer, "reapproximate"):
            # We use fast mode, so we fix it and disable dynamic recomputation
...
def collect_shadow_outputs(model: nn.Module,
    ...
    """
    Randomly sample the same number of member / non-member samples
    from the Shadow HGCN, and return softmax probabilities (2k, C)
    together with corresponding labels y (2k,).

    - model.forward only takes data.x
...
def train_attack_model(
    ...
    """
    Train the attack model and evaluate AUC/F1 on the shadow-test set.
    Supports a specified device.
    The prediction/label shape mismatch issue has been fixed.
    """
    ...
    # Do not unsqueeze; keep shape as [N]
    ...
    for _ in range(epochs):
        ...
        if pred.dim() == 2 and pred.size(1) == 1:
            pred = pred.squeeze(1)   # convert to [N]
...
def membership_inference_hgcn(
    ...
    """
    Run the complete MIA pipeline on HGCN:
      0) Build a config synchronized with main() to ensure all required
         HyperGCN initialization fields are present
      1) Split member vs non-member → Data(x, y, train_mask, test_mask)
      2) Multiple shadow rounds: instantiate Shadow HGCN →
         train_shadow_model → collect_shadow_outputs
      3) Concatenate all shadow outputs → train_attack_model →
         print Shadow AUC/F1
      4) Run the same inference on target_model →
         print Target AUC/F1 → return results
    """
    # 0) Isomorphic config
    cfg = SimpleNamespace()
    ...
    for _ in range(num_shadows):
        ...
        # Ensure Laplacian is on the same device
    ...
    # 3) Train attack model & print shadow metrics

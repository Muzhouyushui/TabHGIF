...
"""
Credit + HGNN + Column Unlearning FT baselines (Edited Hypergraph)
...
Output style:
  edit / update / total / test_acc / train_acc / mia_overall / mia_deleted
"""
...
def freeze_all_but_head_hgnn(model: nn.Module):
    """
    HGNN_implicit 通常有 hgc1/hgc2，把 hgc2 当作 head。
    """
...
def run_one(args, run_id):
    ...
    # 原始测试评估（可选显示）
    test_obj_raw = {
        "x": fts_test, "y": lbls_test,
        "H": H_test, "dv_inv": dv_test, "de_inv": de_test
    }
    try:
    ...
    # train edited
    # Credit 版本 delete_feature_columns 返回顺序通常是 (X_zeroed, new_hyperedges, H_tensor_new)
    ...
    for col in cols_to_unlearn:
        ...
        # 常见格式：(X_zeroed, hyperedges_new, H_tensor_new)
...
# =========================================================
def get_args():
    p = argparse.ArgumentParser("Credit HGNN FT baselines on edited hypergraph (column unlearning)")
    ...
    p.add_argument("--out_csv", type=str, default="ft_hgnn_col_credit_results.csv")

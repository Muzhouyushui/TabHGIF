...
# ----------------------------------------------------------------------
# 1) Utility function: convert hyperedges into sparse H (N, E),
#    then transpose to (E, N) for HGAT
# ----------------------------------------------------------------------
...
# ----------------------------------------------------------------------
# 2) Wrapper: wrap HGAT into an interface that only takes x,
#    so the existing HGNN/HGCN MIA logic can be reused
# ----------------------------------------------------------------------
class _HGATWrapper(nn.Module):
    ...
    def forward(self, x, *_):                     # ignore dv_inv / de_inv
        return self.core(x, self.H)

# ----------------------------------------------------------------------
# 3) Shadow-HGAT training (minimal version)
# ----------------------------------------------------------------------
...
# ----------------------------------------------------------------------
# 4) Main MIA function for the HGAT version
# ----------------------------------------------------------------------
def membership_inference_hgat(
    ...
    """
    Full pipeline:
      ① Construct sparse H → (E, N)
      ② Randomly split member / non-member
      ③ Multiple rounds of Shadow-HGAT → collect_shadow_outputs
      ④ Train the attack model AttackModel
      ⑤ Optionally evaluate on the target_model
    """
    # —— Basic configuration —— #
    device = device or torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    ...
    # Dummy dv/de, only placeholders to satisfy the call signature of collect_shadow_outputs
    ...
    # —— Shadow phase —— #
    from MIA.MIA_utils import collect_shadow_outputs, train_attack_model  # keep original implementation

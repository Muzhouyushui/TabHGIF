...
class HyperGraphConvolution(nn.Module):
    ...
    def forward(self, structure, X, mediators: bool):
        ...
        if self.reapproximate:
            # Dynamically recompute Laplacian:
            # structure stores the hyperedge list
...
def laplacian(edge_list, X: np.ndarray, mediators: bool = True):
    ...
    and then construct the corresponding "expansion" graph.
    """
    rows, cols, weights = [], [], {}
    ...
    for e in edge_list:
        # e is already a list of integer node indices
        if len(e) < 2:
            continue
        proj = X[e] @ rv
...
class HyperGCN(nn.Module):
    ...
    Official HyperGCN network implementation:
      ...
      - args.fast  = whether to precompute the Laplacian
                     (fast=True computes once; fast=False recomputes dynamically)
    ...
    def __init__(self, num_nodes, edge_list, X_init, args):
        ...
        # Hidden-layer dimensions: d -> 2^{depth-1 + offset} -> ... -> c
        ...
        # fast mode: compute sparse Laplacian only once;
        # otherwise recompute dynamically
        if args.fast:
            reapp    = False
        ...
        # All layers use the same reapproximate / use_cuda flags
    ...
    def forward(self, X):
        ...
        # The final layer outputs raw logits, and log_softmax is applied outside

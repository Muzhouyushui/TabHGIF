...
def main():
    ...
    for fold, (tr_idx, te_idx) in enumerate(skf.split(X, y), start=1):
        ...
        df_raw = pd.read_csv(data_path).reset_index(drop=True)

        # 3) 按值删除：从原始数据中找出 A1 == 'b' 的样本，并映射到 tr_idx
        credit_cols = [f"A{i}" for i in range(1, 16)] + ["label"]
        df_raw = pd.read_csv(data_path, header=None, names=credit_cols).reset_index(drop=True)
        ...
        # 找出原始数据中满足条件的全局索引
        matched_global_idx = df_raw[df_raw[delete_col] == delete_val].index.tolist()

        # 映射到训练集内部索引（tr_idx 是全数据中的索引，X_tr 是子集）
        matched_in_train = [i for i, idx in enumerate(tr_idx) if idx in matched_global_idx]

        if len(matched_in_train) == 0:
            raise ValueError(f"[Unlearning] 找不到满足 {delete_col} == {delete_val} 的训练样本")

        # 不限制数量：删除所有满足条件的样本
        forget_idx = matched_in_train

        # 得到保留样本
        retain_idx = np.setdiff1d(np.arange(len(tr_idx)), forget_idx)

...
class HyperGraphConvolution(nn.Module):
    """
    单层 HyperGCN 卷积——可选择在每次前向时重算 Laplacian（reapproximate=True）或复用预计算结果。
    """
    ...
    def forward(self, structure, X, mediators: bool):
        # X: (n_nodes, in_feats)
        HW = X @ self.weight  # 线性变换
        if self.reapproximate:
            # 动态重算 Laplacian：structure 存的是超边列表
            A = laplacian(structure, HW.detach().cpu().numpy(), mediators)
        else:
            # 复用预计算的稀疏 Laplacian tensor
            A = structure
...
def ssm_to_torch(mat: sp.csr_matrix) -> torch.sparse.FloatTensor:
    """把 Scipy 的 CSR 矩阵转成 PyTorch 的 sparse_coo_tensor。"""
...
def laplacian(edge_list, X: np.ndarray, mediators: bool = True):
    """
    HyperGCN 用到的 Laplacian 近似：
    对每个超边随机投影找 supremum/infimum，再构造“张弛”图。
    """
    rows, cols, weights = [], [], {}
    # 随机投影向量
...
class HyperGCN(nn.Module):
    ...
    官方 HyperGCN 网络实现：
      ...
      - args.depth = 层数
      - args.c     = 类别数
      - args.fast  = 是否预计算 Laplacian（fast=True 一次性算好，fast=False 动态重算）
      - args.mediators = 是否在每条超边中加入中介节点
    """
    def __init__(self, num_nodes, edge_list, X_init, args):
        ...
        # fast 模式：只算一次稀疏 Laplacian；否则动态重算
        if args.fast:
            reapp    = False
            structure = laplacian(edge_list, X_init, args.mediators)
        else:
            reapp     = True
            structure = edge_list  # 传入原始超边列表

        self.structure = structure
        # 每层都用同一份 reapproximate / use_cuda 标志
    ...
    def forward(self, X):
        ...
        # 最后一层输出 raw logits，再在外部配合 NLLLoss 使用 log_softmax

...
This module is mainly used to load data from a CSV file and preprocess it:
  - Apply One-Hot encoding to discrete attributes;
  - Apply standardization to continuous attributes;
  - Separate labels and features;
  - Construct the dictionary required for hyperedge construction.
...
def generate_hyperedge_dict_bank(
    ...
    """
    Simplified hyperedge generation: all nodes share a single hyperedge.

    Parameters:
      df                      (DataFrame): Input data
      categorical_cols        (list): Original parameter kept, unused
      max_nodes_per_hyperedge (int): Original parameter kept, unused
      device                  (torch.device): Original parameter kept, unused

    Returns:
      hyperedges (dict): Contains only one hyperedge, with key ('all_nodes',)
                         and value equal to the list of all node indices.
    """
    # Get the index list of all nodes
    all_indices = df.index.tolist()
    # Only one hyperedge, mapping all nodes
    hyperedges = {('all_nodes',): all_indices}
    return hyperedges

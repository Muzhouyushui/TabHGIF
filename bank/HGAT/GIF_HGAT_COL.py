...
def delete_feature_columns_hgat(
    ...
    2) Call rebuild_structure_after_column_deletion to delete hyperedges + rebuild H
    ...
    for col in column_names:
        ...
        if idxs:
            ...
            print(f"[warn] '{col}' did not match any encoded feature")
...
def get_grad_hgat_col(model, data, unlearn_info):
    """
    Compute the gradient difference for column deletion, and print the number of zeroed columns and removed hyperedges.
    ...
    # —— Post-deletion —— #
    # 1) Zero out the corresponding columns in the feature matrix
    x2 = x.clone()
    x2[:, deleted_idxs] = 0.0
    print(f"[ColumnDeletion] Zeroed feature columns: {len(deleted_idxs)} → {deleted_names}")

    # 2) Rebuild hypergraph H2 and print
...
def approx_gif_col(model, data, unlearn_info,
    ...
    GIF update for column deletion:
      ...
      2) Use LiSSA iteration to approximate H^{-1}·g_del_diff
      3) Apply it to the model parameters
    """
    t0 = time.time()
